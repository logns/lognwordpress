...
class Tm_Builder_Module_Slider_Item extends Tm_Builder_Module {
		...
		$this->advanced_options = array(
			...
			'button' => array(
				'button' => array(
					...
					'css'      => array(
						'main' => "{$this->main_css_element}.tm_pb_slide .tm_pb_button",
					),
				),
			),
		);
	...
	function get_fields() {
		$fields = array(
			...
			'button_text' => array(
				...
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'Define the text for the slide button', 'tm_builder' ),
			),
			'button_link' => array(
				'label'           => esc_html__( 'Button URL', 'tm_builder' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'Input a destination URL for the slide button.', 'tm_builder' ),
			),
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		if ( '' !== $this->_var( 'heading' ) ) {
			if ( '#' !== $this->_var( 'button_link' ) ) {
				$this->_var( 'heading', sprintf( '<a href="%1$s">%2$s</a>',
					esc_url( $this->_var( 'button_link' ) ),
					$this->_var( 'heading' )
				) );
			}
		...
		if ( '' !== $this->_var( 'button_text' ) ) {
			...
			$this->_var(
				'button',
				sprintf(
					'<a href="%1$s" class="tm_pb_more_button tm_pb_button%3$s%5$s"%4$s>%2$s</a>',
					esc_url( $this->_var( 'button_link' ) ),
