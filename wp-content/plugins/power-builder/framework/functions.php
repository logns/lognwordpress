...
function tm_builder_include_categories_option( $args = array() ) {
	...
	$output = "\t" . "<% var tm_pb_include_categories_temp = typeof tm_pb_include_categories !== 'undefined' ? tm_pb_include_categories.split( ',' ) : []; %>" . "\n";
	...
	foreach ( $cats_array as $category ) {
		$contains = sprintf(
			'<%%= _.contains( tm_pb_include_categories_temp, "%1$s" ) ? checked="checked" : "" %%>',
	...
	$output = '<div id="tm_pb_include_categories">' . $output . '</div>';
...
function tm_builder_get_builder_post_types() {
	return apply_filters( 'tm_builder_post_types', array(
		'page',
		'project',
		'tm_pb_layout',
		'post',
...
if ( ! function_exists( 'tm_pb_add_builder_page_js_css' ) ) {
	...
	function tm_pb_add_builder_page_js_css(){
		...
		wp_enqueue_style( 'font-awesome', TM_BUILDER_URI . '/framework/admin/assets/css/font-awesome.min.css', array(), '4.5.0' );
		...
		$scheme = 'http://';
		$google_maps_domain = 'maps';
		if ( is_ssl() ) {
			$scheme = 'https://';
			$google_maps_domain = 'maps-api-ssl';
		}
		...
		$google_maps_url = add_query_arg(
			...
			sprintf( "{$scheme}%s.google.com/maps/api/js", $google_maps_domain )
		);

		wp_enqueue_script( 'google-maps-api', esc_url( $google_maps_url ), array(), '3', true );
...
function tm_pb_pagebuilder_meta_box() {
	...
	printf(
		...
		esc_html__( 'Insert Columns', 'tm_builder' ),
	...
	printf(
		...
		esc_html__( 'Desktop', 'tm_builder' ),
		esc_html__( 'Tablet', 'tm_builder' ),
		esc_html__( 'Smartphone', 'tm_builder' )
	);
